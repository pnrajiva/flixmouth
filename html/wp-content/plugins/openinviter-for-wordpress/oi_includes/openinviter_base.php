...
abstract class OpenInviter_Base
	 ...
	 * from a HTML bulk and adds the user and password fields
	 * if provided
	 * 
	 * @param string $string_bulk The HTML bulk from which the fields are extracted
	 * @param string $name_variable_user The name of the user field
	 * @param string $user The value of the user field
	 * @param string $name_variable_pass The name of the password field
	 * @param string $pass The value of the password field
	...
	protected function parseCSV($file, $delimiter=',')
		...
		foreach ($lines as $line)
			...
			$_res=array();
			foreach ($field_names as $key => $f) $_res[$f] = $fields[$key];
			$res[] = $_res;
			}
	 ...
	 */
	protected function followLocation($result)
		{
	 ...
	 * 
	 * @param mixed $cookie_file The cookie file to load cookie from or FALSE if starting a new session
	 */
	...
	protected function get($url,$follow=false,$header=false,$quiet=true,$referer=false,$headers=array())
		...
		if ($this->settings['transport']=='curl')
			...
			if ($follow)
				{
				$new_url=$this->followLocation($result);
				if (!empty($new_url))
